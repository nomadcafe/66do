...
const DomainTable = memo(function DomainTable({ domains, transactions = [], metricsTransactions, onEdit, onDelete, onView, onUpdateDomain }: DomainTableProps) {
  ...
  // 判定收在 lib/expiryBadge，和卡片视图共用——手机上列表被强制成卡片视图，
  // 两边各写一套的话，同一个域名在手机和电脑上说的话就可能不一样。
  const getExpiryStatus = (domain: DomainWithTags) => {
    const badge = getExpiryBadge(domain);
    if (!badge) return null;
    return {
      text: badge.expired ? t('domainList.table.expiredText') : `${badge.days}d`,
      color: EXPIRY_TONE_CLASS[badge.tone],
    };
  };
